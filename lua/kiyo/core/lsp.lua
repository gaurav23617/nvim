...
-- Keymap for inc-rename.nvim
local keys = keys or {} -- initialize if not already declared
keys[#keys + 1] = {
  "<leader>cr",
  function()
...
-- Apply the keys if using lazy.nvim or custom mapping logic
for _, key in ipairs(keys) do
  vim.keymap.set("n", key[1], key[2], { expr = key.expr, desc = key.desc })
end
